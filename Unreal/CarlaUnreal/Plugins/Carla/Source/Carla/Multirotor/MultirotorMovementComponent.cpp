...
void UMultirotorMovementComponent::Initialize(AMultirotorPawn* Owner)
{
    ...
    for (size_t i = 0; i < RotorSetups.Num(); ++i) {
        ...
        if (RotorMesh) {
            ...
            UE_LOG(LogCarla, Warning, TEXT("Error: Could not find Prop%d element"), i);
        }
    }
}
...
void UMultirotorMovementComponent::SetRotorInput(const TArray<float>& Throttle)
{
    ...
    {
        UE_LOG(LogCarla, Warning, TEXT("Got rotor command with %d inputs (expected %d)"), Throttle.Num(), Rotors.Num());
        return;
    }
...
void UMultirotorMovementComponent::UpdatePhysics()
{
    for (auto Rotor : Rotors) {
        if (IsValid(Rotor)) {
            ...
            UE_LOG(LogCarla, Warning, TEXT("Can't update rotor physics, rotor component is not valid"));
        }
    }
}
