...
class ROS2
{
  ...
  // callbacks
  void AddVehicleActorCallback(void* actor, std::string ros_name, VehicleActorCallback callback);
  void AddMultirotorActorCallback(void* actor, std::string ros_name, MultirotorActorCallback callback);
  void RemoveVehicleActorCallback(void* actor);
...
void ProcessDataFromCollisionSensor(
  ...
  std::shared_ptr<CarlaEgoVehicleControlSubscriber> _vehicleController;
  ...
  std::unordered_map<void *, VehicleActorCallback> _vehicle_actor_callbacks;
