...
void URotorPhysics::CalculateMaxThrust()
{
    ...
    const float MaxSpeed = MaxRevolutionsPerSecond * 2.0F * M_PI; // radians / sec
    ...
    MaxTorque = Setup->TorqueCoefficient * AirDensity * NSquared * static_cast<float>(pow(Setup->PropellerDiameter, 5.0F)) / (2.0F * M_PI);
}
