    ...
    if (ROS2Carla->IsEnabled())
    {
      ...
      if (RosName == id) {
        if(RosName.find("vehicle") != std::string::npos)
        {
          ...
          ROS2Carla->AddActorRosName(static_cast<void*>(&Actor), VehicleName);
        }
        else
        {
          ...
          if (pos != std::string::npos) {
            ...
            ROS2Carla->AddActorRosName(static_cast<void*>(&Actor), lastToken);
          }
        }
      } else {
        ROS2Carla->AddActorRosName(static_cast<void*>(&Actor), RosName);
      }
...
void UActorDispatcher::OnActorDestroyed(AActor *Actor)
{
  ...
  if (ROS2Carla->IsEnabled())
  {
    ROS2Carla->RemoveActorRosName(reinterpret_cast<void *>(Actor));
  }
  #endif
}
